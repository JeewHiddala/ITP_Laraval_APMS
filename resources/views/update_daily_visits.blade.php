...
<head>
    ...
    <title>Update | Daily Visits</title>   
...
<body>
  ...
  <img src="{{ asset('/images/logo.png') }}" alt="logo" width="110" height="100" style="float:left; margin-top:-2.2% ;padding-left:0.5%">
    ...
    <form method="post" action = "/updatevisit2">
        {{csrf_field()}}

        <input type="hidden" name="visit_id" value="{{$data->id}}" class = "visit_id form-control" />
        <p style="text-align:left">Date</p>
        <input type="date" class = "date form-control" style="width:250px" value="{{$data->date}}" name="date"  placeholder = "" /><br>
        <p style="text-align:left">Departure Time</p>
        <input type="time" class = "departure form-control" style="width:250px" value="{{$data->departure}}" name="departure" placeholder = "" /><br>
        <p style="text-align:left">Arrival Time</p>
        <input type="time" class = "arrival form-control" style="width:250px" value="{{$data->arrival}}" name="arrival" placeholder = "" /><br>
        <p style="text-align:left">Purpose</p>
        <textarea  class ="purpose form-control" value="{{$data->purpose}}" style="width:550px" rows ="4" name="purpose" placeholder = "" >{{$data->purpose}}</textarea><br>

        <select name="driver_id"  class="driver form-control" style="width:250px" id="driver_id">
            <option value="{{$data->driver_id}}" hidden>{{$data->name}}</option>
        ...
        <select name="vehicle_id" class="vehicle form-control" style="width:250px" id="vehicle_id">
            <option value="{{$data->vehicle_id}}" hidden>{{$data->vehi_reg_no}}</option>
        ...
        <input type="number" class = "fuel form-control" style="width:250px" value="{{$data->fuel}}" name="fuel" placeholder = ""/><br>

        <input  type="submit" class="btn btn-primary" value="Save" onclick="return confirm('Are you sure you want to update?')"/>&nbsp;&nbsp;
        <a href="/visits" class="btn btn-danger" >Cancel</a>
        </form>
        </div></div>
