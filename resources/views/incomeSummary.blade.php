...
<div class="container-lg pt-5">
    <ul class="nav nav-tabs" role="tablist">
      <li class="nav-item">
        ...
        <li class="nav-item">
          <a class="nav-link active {{request () -> is('/incomeSummary') ? 'active' : null }}" href="{{url('/incomeSummary')}}"
    ...
    <br>
        ...
        <form method="GET" action="/incomeSummary">
...
<table class="table table-dark">
        ...
        @foreach($data as $ex)
        <tr>
            <td>{{$ex->receipt_number}}</td>
            <td>{{$ex->category}}</td>
            <td>{{$ex->description}}</td>
            <td>{{$ex->date_of_transaction}}</td>
            <td>{{$ex->amount}}</td>
            <td>{{$ex->discount_given}}</td>
        </tr>
        @endforeach
        <tr>
