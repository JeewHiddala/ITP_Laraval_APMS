...
{
    //
    function IncomePDF($fromDate, $toDate)
    {
        // dd($fromDate);
        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($this->GeneratePDF($fromDate, $toDate));
        $docName = "Income Report"; //Giving a name to the report appear after download
    ...
    function GeneratePDF($fromDate, $toDate)
    {
        ...
        <p>Total Income for the given period - From <b><i>'.$printFromDate.'</i></b> To <b><i>'.$printToDate.'</i></b> </p><br>
        <table width="100%" style="border-collapse: collapse; border: 0px;">
            ...
            $output .= '
            <tr>
                <td style="border: 1px solid; padding:4px; text-align:left;">'.$data->receipt_number.'</td>
                ...
                <td style="border: 1px solid; padding:4px; text-align:right;">'.$data->amount.'</td>
                <td style="border: 1px solid; padding:4px; text-align:right;">'.$data->discount_given.'</td>            
            ';
            }
            $output .= '
            <tbody></tbody>
            <tfoot>
                <tr id = "rowid">
                    ...
                    <th id = "total_cost" class = "id" style="border: 1px solid; padding:4px; text-align:right;">LKR'.$totalIncome.'.00</th>
